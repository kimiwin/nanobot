...
# ---------------------------------------------------------------------------
# MiniMax OAuth commands
# ---------------------------------------------------------------------------

@app.command("oauth")
def oauth_command(
    action: str = typer.Argument(..., help="Action: login/status/refresh/token"),
    region: str = typer.Option("cn", "-r", "--region", help="Region: cn/global"),
):
    """MiniMax OAuth authentication."""
    import os
    from nanobot.oauth.minimax import get_minimax_access_token, login_minimax
    
    if action == "status":
        token_file = os.path.expanduser("~/.nanobot/.minimax_token")
        if os.path.exists(token_file):
            console.print("✅ MiniMax OAuth configured")
        else:
            console.print("❌ MiniMax OAuth not configured")
            console.print("Run: nanobot oauth login")
    
    elif action == "login":
        console.print("🌀 MiniMax OAuth Login")
        token_data = asyncio.run(login_minimax(region))
        console.print(f"\n✅ Login successful!")
        console.print(f"Access Token: {token_data['access'][:20]}...")
        console.print(f"Expires: {token_data['expires']}s")
    
    elif action == "refresh":
        console.print("🔄 Refreshing token...")
        token = asyncio.run(get_minimax_access_token(force_refresh=True))
        console.print(f"✅ New token: {token[:20]}...")
    
    elif action == "token":
        console.print("🔑 Getting access token...")
        token = asyncio.run(get_minimax_access_token())
        console.print(token)
